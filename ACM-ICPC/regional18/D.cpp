...
#define erase_all( cont, value ) cont.erase( remove( cont.begin(), cont.end(), value ), cont.end() )
...
int main(void){
  ...
  REP( i, n ) cin >> vs[i];
  REP( i, n ){
    int pos = vs[i].find( '+' ), atsign = vs[i].find( '@' );
    string t, u;
    if( pos == string::npos ){ // not found
      t = vs[i].substr( 0, atsign );
    }
    else t = vs[i].substr( 0, pos );
    u = vs[i].substr( atsign );
    erase_all( t, '.' );
    values.emplace( t + u );
  }
