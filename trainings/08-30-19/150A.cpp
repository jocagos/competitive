#include <bits/stdc++.h>
constexpr long long MAXN = 1000001;
using namespace std;

map<int,bool> sieve; // Check list
vector<int> primes;

void atkin(){
  if (MAXN > 2)
    primes.emplace_back(2);
  if (MAXN > 3)
    primes.emplace_back(3);

  for (int i = 0; i < MAXN; i++)
    sieve[i] = false;

  for (int x = 1; x * x < MAXN; x++) {
    for (int y = 1; y * y < MAXN; y++) {
      int n = (4 * x * x) + (y * y);
      if (n <= MAXN && (n % 12 == 1 || n % 12 == 5))
	sieve[n] ^= true;
      n = (3 * x * x) + (y * y);
      if (n <= MAXN && n % 12 == 7)
	sieve[n] ^= true;
      n = (3 * x * x) - (y * y);
      if (x > y && n <= MAXN && n % 12 == 11)
	sieve[n] ^= true;
    }
  }
  for (int r = 5; r * r < MAXN; r++) {
    if (sieve[r])
      for (int i = r * r; i < MAXN; i += r * r)
	sieve[i] = false;
  }

  for (int a = 5; a < MAXN; a++)
    if (sieve[a])
      primes.emplace_back(a);
}

long long s( long long n ){ return n * n; }
bool is_prime( int n ){
  if( n < MAXN ) return binary_search( begin( primes ), end( primes ), n );
  else{
    
  }
}

int main(){
  atkin();
  ios_base::sync_with_stdio(0);
  cin.tie(0);
  cout.tie(0);
  reverse( begin( primes ), end( primes ) );
  primes.emplace_back( 1 );
  reverse( begin( primes ), end( primes ) );
  int n;
  cin >> n;
  if( binary_search( begin( primes ), end( primes ), n ) or binary_search( begin( primes ), end( primes ), (int)sqrt( n ) ) ) cout << "1\n0\n";
  else{
    
  }
  return 0;
}
