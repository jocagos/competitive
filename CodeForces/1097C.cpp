...
int main(void){
  ...
  REP( i, n ){
    ...
    // cout << ">" << s << "< is " << (isValid( r + s ) or isValid( s + r ) ? "valid" : "not valid") << endl;
    if( isValid( r + s ) or isValid( s + r ) ){
      // cout << ">" << s << "< is valid" << endl;
      REP( j, (int)s.length() ) t += ( s[j] == '(' ? 1 : -1 );
      brackets[t]++;
    }
  }
  int pairs = brackets[0] / 2;
  brackets.erase( 0 );
  for( auto it = brackets.begin(); it != brackets.end(); ++ it ){
    if( brackets.find( -it->first ) == brackets.end() ) continue;
    else{
      int m = min( brackets[it->first], brackets[-it->first] );
      pairs += m;
      brackets[it->first] -= m;
      brackets[-it->first] -= m;
    }
  }
  // for( auto& [key, val] : brackets ){
  //   if( brackets.find( -key ) == brackets.end() ) continue;
  //   else{
  //     int m = min( brackets[key], brackets[-key] );
  //     pairs += m;
  //     brackets[key] -= m;
  //     brackets[-key] -= m;
  //   }
