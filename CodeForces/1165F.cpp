...
constexpr int MAXN = 2 * (1e5 + 100);
constexpr int DUMMY = -1;
int m, n;
vector<ll> copies;
vtwin<ll> offers( MAXN );

bool able( int when ){
  vector<ll> least( n, DUMMY  );
  REP( i, m ){
    if( get<0>( offers[i] ) <= when )
      least[ get<1>( offers[i] ) ] = max( least[ get<1>( offers[i] ) ], get<0>( offers[i] ) );
  }
  vector<vi> offset( MAXN );
  REP( i, n ) if( least[i] != DUMMY ) offset[least[i]].EB( i );
  vector<ll> need( all( copies ) );
  int money = 0;
  REP( i, when + 1 ){
    ++ money;
    if( i > MAXN ) continue;
  ...
  ll ans = accumulate( all( need ), 0 ) * 2 <= money;
  return ans;
}
