...
char par[12][12][12];
bool v[12][12][12];
void dfs( int x = 0, int y = 0, int z = 0 ){
  if( v[x][y][z] or par[x][y][z] == '#' ) return;
  v[x][y][z] = V;
  c ++;
  if( x > 0 ){
    if( !v[x - 1][y][z] and par[x - 1][y][z] == '.' ) dfs( x - 1, y, z );
  }
  if( x + 1 < n ){
    if( !v[x + 1][y][z] and par[x + 1][y][z] == '.' ) dfs( x + 1, y, z );
  }
  if( y > 0 ){
    if( !v[x][y - 1][z] and par[x][y - 1][z] == '.' ) dfs( x, y - 1, z );
  }
  if( y + 1 < m ){
    if( !v[x][y + 1][z] and par[x][y + 1][z] == '.' ) dfs( x, y + 1, z );
  }
  if( z > 0 ){
    if( !v[x][y][z - 1] and par[x][y][z - 1] == '.' ) dfs( x, y, z - 1 );
  }
  if( z + 1 < k ){
    if( !v[x][y][z + 1] and par[x][y][z + 1] == '.' ) dfs( x, y, z + 1 );
  }
