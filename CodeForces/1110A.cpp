...
constexpr int MAXK = 101000, MAXB = 110;
int b, k;
vi kint( MAXK ), parity( MAXK );

int main(void){
  fastio;
  cin >> b >> k;
  REP( i, k ) cin >> kint[i];
  REP( i, k ) parity[i] = ( i + 1 != k ? (b % 2) : 1) * (kint[i] % 2);
  FOR( i, 1, k ) parity[i] = (parity[i - 1] + parity[i]) % 2;
  bool even = parity[k - 1];
  if( even ) cout << "odd\n";
  else cout << "even\n";
  return 0;
}
