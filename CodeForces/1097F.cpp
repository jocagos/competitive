...
constexpr int MAXN = 10010, MAXQ = 100100;
int n, q;
vquad<int> queries( MAXQ );
...
int main(void){
  ...
  REP( i, q ){
    ...
    if( a == 2 or a == 3 ) cin >> d;
    // else d = -1;
    // queries[i] = MT( a, b, c, d );
    if( a == 1 ) ms[b - 1] = { c };
    else if( a == 2 ){
      multiset<int> tmp( ms[c - 1] );
      for( auto p : ms[d - 1] ) tmp.insert( p );
      ms[b - 1] = tmp;
    }
    else if( a == 3 ){
      multiset<int> tmp;
      for( auto p : ms[c - 1] ){
	for( auto q : ms[d - 1] ) tmp.insert( gcd( p, q ) );
      }
      ms[b - 1] = tmp;
    }
    else if( a == 4 ){
      cout << ms[b - 1].count( c ) % 2;
    }
  }
  cout << '\n';
  return 0;
