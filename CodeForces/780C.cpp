...
int n, maxC = 1;
vector<vi> adjList( 200010 );
vi colors( 200010, DUMMY );

int dfs( int u = 1, int color = -1, int m = -1 ){
  int p = 1;
  for( int v : adjList[u] ){
    if( m != v ){
      while( p == colors[u] or color == p ) ++ p;
      colors[v] = p;
      ++ p;
      dfs( v, colors[u], u );
    }
  }
  maxC = max( maxC, colors[u] );
}
