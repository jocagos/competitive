...
// bool backtrack( int taken, int bitmask ){
//   if( taken == 0 ){
//     int t = 0;
//     for( int i = 0; i < n; ++ i ) if( bitmask & (1 << i) ) t += rot[i];
//     return memo[cntSetBits(bitmask)][bitmask] = (s - t == t);
//   }
//   bool& ans = memo[taken][bitmask];
//   for( int i = 0; i < n; ++ i ){
//     if( ans ) return ans;
//     if( !(bitmask & (1 << i)) ) ans |= backtrack( taken - 1, bitmask | (1 << i) );
//   }
//   return ans;
// }
bool sol;
void backtrack( int idx, ll pos ){
  if( sol ) return;
  if( idx >= n ){
    if( pos == 0 ) sol = true;
    return;
  }
  backtrack( idx + 1, (360 + pos + rot[idx])%360 );
  backtrack( idx + 1, (360 + pos - rot[idx])%360 );
}
