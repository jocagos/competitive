...
// -1 = Tobby, 0 = Draw, 1 = Naebbirac
inline short ans( int a, int b, char * s, char * t ){
  // D = 3, C = 2, B = 1
  int left = s[0] - 'A', right = t[0] - 'A';
  if( left == right ) return ( a < b ? -1 : (a > b ? 1 : 0) );
  else{
    if( left == 1 ){ // battleship - tobby
      if( right == 3 ){ // destroyers - naebbirac
	if( b < 2 * a ) return -1;
	else return 1;
      }
      else{ // cruisers
    ...
    else if( left == 2 ){ // cruisers - tobby
      if( right == 1 ){ // battleships - naebbirac
	if( b < 2 * a ) return -1;
	else return 1;
      }
      else{ // destroyers
    ...
    else if( left == 3 ){ // destroyers - tobby
      if( right == 1 ){ // battleships - naebbirac
	if( b < 2 * a ) return 1;
	else return -1;
      }
