...
string S;
vector<vector<string>> rhymes( MAXN );
int E, P;
...
int main(void){
  ...
  REP( i, E ){
    ...
    string t;
    while( iss >> t ) rhymes[i].emplace_back( t );
    sort( all( rhymes[i] ) );
  }
  getline( cin, line );
  P = stoi( line );
  REP( i, P ){
    ...
    string t;
    while( iss >> t );
    // cerr << "Phrase " << i << " has last word = " << t << endl;
    bool found = false;
    for( auto words : rhymes ){
      if( found ) break;
      int a = 0, b = 0;
      for( auto word : words ){
	if( found ) break;
	if( S.find( word ) != string::npos ) a = 1;
	if( t.find( word ) != string::npos ) b = 1;
      }
      found = ( a + b ) / 2;
    }
    if( found ) cout << "YES\n";
