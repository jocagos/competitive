...
int main(void){
  ...
  fastio;
  cin >> m >> n >> o;
  vector<vector<unsigned long long>> A( m, vector<unsigned long long>( n ) ), B( n , vector<unsigned long long>( o ) ), C( m , vector<unsigned long long>( o ) );
  for( int i = 0; i < m; ++ i ) for( int j = 0; j < n; ++ j ) cin >> A[i][j];
  for( int i = 0; i < n; ++ i ) for( int j = 0; j < o; ++ j ) cin >> B[i][j];
  REP( i, m ){
    REP( j, o ){
      ll s = 0;
      REP( k, n ) s += A[i][k] * B[k][j];
      C[i][j] = s;
    }
  }
  REP( i, m ){
    REP( j, o ) cout << C[i][j] << ( j + 1 == o ? "\n" : " " ); 
  }
  return 0;
}
