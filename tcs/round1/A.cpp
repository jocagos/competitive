...
bitset<MAXN> onoff( 0 );
vector<pair<dd, int>> intervals( MAXN );
...
int main(void){
  ...
  REP( i, n ){
    cin >> a[i];
    intervals[i] = { { max( 0.0, i * d - a[i] / 2 ), min( i * d + a[i] / 2, maxL ) }, i };
  }
  sort( justN( intervals, n ), []( pair<dd, int>& left, pair<dd, int>&right ){ return ( left.fi.fi != right.fi.fi ? left.fi.fi < right.fi.fi : left.fi.se > right.fi.se ); } );
  double right = 0;
  int farthest;
  bool done = false;
  for( int i = 0, j = 0; i < n; ++ i ){
    double tmp = right;
    farthest = -1;
    while( j < n and intervals[j].fi.fi <= right ){
      if( intervals[j].fi.se > tmp ){
	tmp = intervals[j].fi.se;
	farthest = intervals[j].se;
	onoff[intervals[j].se] = 1;
      }
      ++ j;
    }
    if( tmp >= maxL ){
  ...
  if( not done ){
    ...
    cout << onoff.count() << endl;
    REP( i, n ) if( onoff[i] ) cout << i + 1 << " ";
    cout << endl;
    REP( i, n ) if( not onoff[i] ) cout << i + 1 << " ";
    cout << endl;
  }
  return 0;
}
