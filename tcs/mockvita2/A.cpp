...
inline ld d2r( ld d ){ return M_PI * d / 180.0; }
inline ld r2d( ld r ){ return 180.0 * r / M_PI; }
vector<ii> points( 10 );
vector<pair<char, char>> dirs( 10 );
int main(void){
  int n;
  fastio;
  const regex r("(\\d+)(A-Z+),(\\d+)(A-Z+)");
  smatch p;
  cin >> n;
  string s;
  REP( i, n ){
    cin >> s;
    if( regex_search( s, p, r ) ){
      stringstream iss;
      for( int j = 1; i < p.size(); ++ j ){
	cout << "I found " << p[j] << endl;
	iss << p[j];
	if( j == 1 ) iss >> points[i].fi;
	else if( j == 3 ) iss >> points[i].se;
	else if( j == 2 ) iss >> dirs[i].fi;
	else iss >> dirs[i].se;
      }
      cout << "I got a point at (" << points[i].fi << ", " << dirs[i].fi << ") and another at (" << points[i].se << ", " << dirs[i].se << ")" << endl;
    }
  }
  return 0;
}
