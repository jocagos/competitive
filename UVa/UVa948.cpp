...
#define viewStr(s) cerr << #s << ": >" << (s) << "<" << endl
#define view_case(c,x) cerr << "case " << c << ": " << #x << " = " << x << endl
#define view_case_str(c,x) cerr << "case " << c << ": " << #x << " = >" << x << "<" << endl
#define view(x) cerr << #x << ": " << x << endl
...
#define sq(a) (a) * (a)
#define out_case( c, v, caps ) cerr << ( caps ? "C" : "c" ) << "ase " << c << ": " << v << endl
#define fi first
#define se second
#define g0 get<0>
#define g1 get<1>
...
int main(void){
  ...
  while( t -- ){
    ...
    cin >> n;
    m = n;
    auto tmp = found.find( n );
    if( tmp != found.end() ) cout << n << " = " << tmp->se << " (fib)\n";
    else{
      string s( 100, '0');
      // view(s);
      while( m ){
	auto it = upper_bound( justN( fibos, max_fibo ), m );
	s[it - fibos.begin() - 1] = '1';
	// cerr << "it is [" << it - fibos.begin() << ", " << (it != fibos.end() ? to_string( *it ) : "NULL" ) << "]" << endl;
	// view_case_str( tc - t, s );
	it --;
	m -= *it;
      }
      int idx = s.find_last_of('1');
      s = s.substr( 1, idx );
      string _t( s.rbegin(), s.rend() );
      found[n] = _t;
      cout << n << " = " << _t << " (fib)\n";
    }
  }
