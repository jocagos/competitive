...
vtriad<int, int, int> mst( MAXN ), lines( MAXN );
auto lambda = []( triad<int>& left, triad<int>& right ){ return ( get<2>( left ) != get<2>( right ) ? get<2>( left ) < get<2>( right ) : (get<1>( left ) != get<1>( right ) ? get<1>( left ) < get<1>( right ) : get<0>( left ) < get<0>( right ) ) ); };
...
int main(void){
  ...
  while( cin >> n ){
    ...
    cin >> m;
    if( m + k > lines.size() ) lines.resize( m + k + 1 );
    REP( i, m ){
      cin >> a >> b >> c;
      lines[i+k] = MT( c, a, b );
    }
    // sort( justN( lines, m + k ) );
    sort( begin(lines), begin(lines) + m + k );
    int cost = 0;
    UFDS ufds( m + k );
    REP( i, m + k ){
      if( ufds.numDisjointSets() == 1 ) break;
      triad<int> t = lines[i];
      if( not ufds.isSameSet( get<1>( t ), get<2>( t ) ) ){
	cost += get<0>( t );
	ufds.unionSet( get<1>( t ), get<2>( t ) );
      }
    }
    cout << or_cost << '\n' << cost << '\n';
  }
