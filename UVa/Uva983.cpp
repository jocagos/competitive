...
int main(void){
  int n, m;
  bool p = false;
  // pre initialize space for matrices `matrix' and `dp'
  vector<vector<int>> matrix( 1000, vector<int>( 1000, 0 ) );
  vector<vector<ll>> dp( 1000, vector<ll>( 1000, 0LL ) );
  fastio;
  while( cin >> n ){
    // boolean to print after every case a new line
    if( p ) cout << '\n';
    else p = true;
    // get m
    cin >> m;
    // Initialize Submatrix
    vector<vector<ll>> sub( n - m + 1, vector<ll>( n - m + 1, 0LL ) );
    int subI = 0, subJ = 0, subSum = 0;
    // read data
    FORD( i, n - 1, 0 ){
      REP( j, n ){
	cin >> matrix[i][j];
    ...
    // Max 2D range sum for submatrix
    FOR( i, m - 1, n ){
      FOR( j, m - 1, n ){
	// Optionally sum previous values in case there's any
	sub[subI][subJ ++] = dp[i][j] - ( i > m - 1 ? dp[i - m][j] : 0 ) - ( j > m - 1 ? dp[i][j - m] : 0 ) + ( i > m - 1 and j > m - 1 ? dp[i - m][j - m] : 0 );
      }
      // Reset counters
      ++ subI;
      subJ = 0;
    }
    // Printing solution in order (bottom-top, left-right)
    for( int i = sz(sub) - 1; i >= 0; -- i ) for( int j = 0; j < sz(sub[i]); ++ j ) cout << sub[i][j] << '\n', subSum += sub[i][j];
    // Printing final sum
    cout << subSum << '\n';
  }
  return 0;
