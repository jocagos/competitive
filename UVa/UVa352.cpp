...
void bfs( int x, int y ){
  ...
  while( not q.empty() ){
    ii p = q.front(); q.pop();
    v[p.fi][p.se] = V;
    if( p.fi > 0 ) if( g[p.fi - 1][p.se] == '1' and !v[p.fi - 1][p.se] ) q.emplace( p.fi - 1, p.se );
    if( p.se > 0 ) if( g[p.fi][p.se - 1] == '1' and !v[p.fi][p.se - 1] ) q.emplace( p.fi, p.se - 1 );
    if( p.fi + 1 < n ) if( g[p.fi + 1][p.se] == '1' and !v[p.fi + 1][p.se] ) q.emplace( p.fi + 1, p.se );
    if( p.se + 1 < n ) if( g[p.fi][p.se + 1] == '1' and !v[p.fi][p.se + 1] ) q.emplace( p.fi, p.se + 1 );
    if( p.fi > 0 and p.se > 0 ) if( g[p.fi - 1][p.se - 1] == '1' and !v[p.fi - 1][p.se - 1] ) q.emplace( p.fi - 1, p.se - 1 );
    if( p.se > 0 and p.fi + 1 < n ) if( g[p.fi + 1][p.se - 1] == '1' and !v[p.fi + 1][p.se - 1] ) q.emplace( p.fi + 1, p.se - 1 );
    if( p.fi + 1 < n and p.se + 1 < n ) if( g[p.fi + 1][p.se + 1] == '1' and !v[p.fi + 1][p.se + 1] ) q.emplace( p.fi + 1, p.se + 1 );
    if( p.se + 1 < n and p.fi > 0 ) if( g[p.fi - 1][p.se + 1] == '1' and !v[p.fi - 1][p.se + 1] ) q.emplace( p.fi - 1, p.se + 1 );
  }
}
...
int main(void){
  ...
  while( cin >> n ){
    ...
    cout << "Image number " << _ ++ << " contains " << c << " war eagles.\n";
    REP( i, n ) v[i].reset();
    memset( g, 0, sizeof g );
  }
  return 0;
