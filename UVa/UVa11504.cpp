...
int n, m, cc = 0, tsIdx = 0;
int toposort[MAXN] {}, vis[MAXN] {};
vector<vi> g;

void ts( int u ){
  vis[u] = 1;
  for( auto v : g[u] ) if( not vis[v] ) ts( v );
  toposort[tsIdx ++] = u;
}

void dfs( int u ){
  vis[u] = 1;
  for( auto v : g[u] ) if( not vis[v] ) dfs( v );
}
...
int main(void){
  ...
  while( tc -- ){
    ...
    REP( i, n ) if( not vis[i] ) ts( i );
    memset( vis, 0, (sizeof(int) * n) );
    FORD( i, tsIdx - 1, 0 ){
      if( not vis[toposort[i]] ){
	cc ++;
	dfs( toposort[i] );
      }
    }
