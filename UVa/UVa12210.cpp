...
int main(void){
  int t = 1, b, s;
  vector<int> bachelors(10000, 0), spinters(10000, 0);
  fastio;
  while( cin >> b >> s, b | s ){
    cout << "Case " << t++ << ": ";
    int missing = b - s, youngest = 100;
    REP( i, b ){
      cin >> bachelors[i];
      if( bachelors[i] < youngest ) youngest = bachelors[i];
    }
    REP( i, s ) cin >> spinters[i];
    cout << (missing > 0 ? missing : 0) << (missing > 0 ? " " + to_string(youngest) + '\n' : "\n");
  }
  return 0;
}
