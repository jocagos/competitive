...
int n, e, t, m, a, b, c, tc;
vector<vtwin<int>> g( MAXN );
vi dist( MAXN, INF );
std::priority_queue< twin<int>, vtwin<int>, greater<twin<int>> > pq;
// buggy while doing dijkstra OR when counting mice OR both
int main(void){
  fastio;
  cin >> tc;
  while( tc -- ){
    cin >> n >> e >> t >> m;
    // cerr << "CASE #" << _ ++ << endl << "n: " << n << "\te: " << e << "\nt: " << t << "\tm: " << m << endl;
    FOR( i, 1, n + 10 ){
      g[i].clear();
      dist[i] = INF;
    }
