...
constexpr int MAXN = 2100, UNV = 0;
int n, m, dfsc = 1, scc = 0;
vi num( MAXN, 0 ), low( MAXN, 0 );
bitset<MAXN> vis( 0 );
deque<int> st;
vvi g;
...
void tarjan( int u ){
  ...
  vis[u] = 1;
  for( auto v : g[u] ){
    if( num[v] == UNV ) tarjan( v );
    if( vis[v] ) low[u] = min( low[u], low[v] );
  }
  if( low[u] == num[u] ){
    ++ scc;
    while( true ){
      int w = st.back(); st.pop_back(); vis[w] = UNV;
      if( u == w ) break;
    }
  }
}
...
int main(void){
  ...
  fastio;
  while( cin >> n >> m, n | m ){
    fill( justN( num, n ), UNV );
    fill( justN( low, n ), UNV );
    vis.reset();
    st.clear();
    g.assign( n, vi() );
    dfsc = scc = 0;
    ...
    REP( i, n ){
      if( num[i] == UNV ) tarjan( i );
    }
    cout << ( scc == 1 ? "1\n" : "0\n" );
    // if( scc != 1 ) cout << "Case " << idx ++ << ": " << scc << endl;
    // else cout << '\n';
  }
  return 0;
}
