...
bool backtrack( int idx = 0, int ans = 0 ){
  if( possible ) return possible;
  if( idx == 5 ){
    return ans == 23;
  }
  return backtrack( idx + 1, ans + a[idx] ) ||
         backtrack( idx + 1, ans - a[idx] ) ||
         backtrack( idx + 1, ans * a[idx] );
}

int main(void){
  fastio;
  while( cin >> a[0] >> a[1] >> a[2] >> a[3] >> a[4], *a | a[1] | a[2] | a[3] | a[4] ){
    possible = false;
    sort( a, a + 5 );
    do{
      possible = backtrack( 1, a[0] );
    } while( not possible and next_permutation( a, a + 5 ) );
