...
bool mat[20][20];
bitset<30> rw, ld, rd;
void backtrack(int col = 0, int rw = 0, int ld = 0, int rd = 0) {
  if (rw == OK) { ans++; return; }
  int pos = OK bitand (compl(rw bitor ld bitor rd));
  REP( row, n ){
    if( 1 bitand (pos >> row) bitand (mat[row][col]) ){
      backtrack( col + 1, rw bitor (1 << row), (ld bitor (1 << row)) << 1, (rd bitor (1 << row)) >> 1 );
    }
  }
}
