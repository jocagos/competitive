...
constexpr int UNV = 0, INIT = 1000, MAXN = 110;
int g[MAXN][MAXN], b[MAXN][MAXN], d[MAXN], v[MAXN], n;

int bridges( int u, int depth, int parent ){
  int back = INIT, tmp;
  d[u] = depth;
  REP( i, n ){
    if( g[u][i] == 1 ){
      if( !v[i] ){
	v[i] = 1;
	tmp = bridges( i, depth + 1, u );
	if( tmp > depth ) b[u][i] = b[i][u] = 1;
	back = min( back, tmp );
      }
      else if( i != parent ) back = min( back, d[i] );
    }
  }
  return back;
}
...
int main(){
  ...
  while( scanf( "%d", &n ) != EOF ){
    ...
    int ans = 0;
    REP( i, n ) FOR( j, i + 1, n ) ans += b[i][j];
    printf( "%d critical links\n", ans );
    REP( i, n ) FOR( j, i + 1, n ) if( b[i][j] ) printf( "%d - %d\n", i, j );
    printf("\n");
  }
  return 0;
}
