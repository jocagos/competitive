...
constexpr int MAXN = 10010, UNV = 0;
int n, m, t;
vi visit( MAXN ), com( MAXN );
vvi g;

int articulations( int u, int p ){
  int vx = visit[u] = ++t;
  com[u] = 1;
  for( auto v : g[u] ) {
    if( v == p ) continue;
    if( visit[v] ) vx = min( vx, visit[v] );
    else{
      int vy = articulations( v, u );
      vx = min( vx, vy );
      if( vy >= visit[u] ) com[u] ++;
    }
  }
...
public:
  bool operator()( ii left, ii right ){
    if( left.se != right.se ) return right.se > left.se;
...
int main(void){
  fastio;
  while( cin >> n >> m, n | m ){
    std::priority_queue< ii, vector<ii>, Cmp> pq;
    fill( justN( visit, n ), 0 );
    g.assign( n, vi() );
    int x, y;
    while( cin >> x >> y, x >= 0 ){
      g[x].EB( y );
