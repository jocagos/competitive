...
constexpr int MAXN = 10100, MOD = 10000, MAXR = 11;
int l, u, r, tc = 1;
vector<int> R( MAXR );
bool v[MAXN];
...
int main(void){
  ...
  while( cin >> l >> u >> r, l | u | r ){
    // cerr << "CASE: L = " << setw( 4 ) << setfill( '0' ) << l << "\tU = " << setw( 4 ) << setfill( '0' ) << u << "\tR = " << setw( 4 ) << setfill( '0' ) << r << endl;
    // cerr << "\tTC: " << tc ++ << endl << "==================================" << endl;
    memset( v, false, sizeof v );
    int dist = -1;
    REP( i, r ) cin >> R[i];
    // REP( i, r ) cerr << "R[" << i + 1 << "] = " << setw( 4 ) << setfill( '0' ) << R[i] << endl;
    queue<twin<int>> q;
    q.emplace( l, 0 );
    while( not q.empty() ){
      twin<int> tmp = q.front();
      int & g = get<0>( tmp ), & layer = get<1>( tmp );
      q.pop();
      if( g == u ){
	dist = layer;
	break;
      ...
      REP( i, r ){
	if( not v[(g + R[i])%MOD] )
	  q.emplace( (g + R[i])%MOD, layer + 1 );
	  // cerr << "emplaced " << (g + R[i])%MOD << ", " << layer + 1 << " from " << g << "\tq size: " << q.size() << endl;
      }
    }
    cout << "Case " << tc ++ << ": " << (dist != -1 ? to_string( dist ) : "Permanently Locked") << endl;
    // cerr << "==========================================================" << endl;
    // cerr << "==========================================================" << endl;
  }
