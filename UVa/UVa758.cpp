...
constexpr char RED = 'R', GREEN = 'G', BLUE = 'B', EMPTY = '*';
int t, idx_move = 1;
bool game_over = false;
char same[10][15];
bool vis[10][15];

inline void cans( int x, int y, int b, char c, int s ){
  cout << "Move " << idx_move << " at (" << x << ", " << y << "): removed " << b << " balls of color " << c << ", got " << s << " points.\n";
}
  ...
  for( int i = 0; i < 10; ++ i ){
	...
	while( not q.empty() ){
	  ...
	  for( auto& p : n4 ){
	    int a, b;
	    tie( a, b ) = p;
	    if( in( 0, 10, x + a ) and in( 0, 15, y + b ) and not vis[x + a][y + b] and same[x + a][y + b] == col ){
	      q.emplace( x + a, y + b );
...
int main(void){
  ...
  while( getline( cin, line ) ){
    idx_move = 1;
    REP( i, 10 ) REP( j, (int)line.size() ) same[i][j] = line[j];
    while( not game_over ){
      priority_queue<triad<int>> pq;
