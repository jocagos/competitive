...
void fill( int a, int b ){
  ...
  v[a][b] = V;
  for( auto p : n4 ){
    if( in( 0, lines - 1, a + p.fi ) and in( 0, lengths[a + p.fi] - 1, b + p.se ) and (g[a + p.fi][b + p.se] == '-' or g[a + p.fi][b + p.se] == ' ') ){
      fill( a + p.fi, b + p.se );
    }
  }
}
...
int main(void){
  ...
  while( tc -- ){
    ...
    while( getline( cin, line ), line[0] != '_' ){
      // cout << "case " << tc << ", line >" << line << "<" << endl;
      lengths[lines] = line.length();
      REP( i, (int)line.length() ){
	if( line[i] == '*' ){
	  x = lines;
	  y = i;
	}
