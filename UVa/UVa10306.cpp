...
inline ll ssq( ll a, ll b ){ return a * a + b * b; }
constexpr int MAXM = 45, MAXS = 310;
int m, s;
vector<pair<ll, ll>> coins( MAXM );
ll dp[MAXM][MAXS][MAXS];
ll ways( int idx, int val1, int val2 ){
  if( ssq( val1, val2 ) == s ) return 0;
  if( ssq( val1, val2 ) > s or idx < 0 ) return LLINF;
  if( dp[idx][val1][val2] == 0 ) return dp[idx][val1][val2] = min( ways( idx - 1, val1, val2 ), 1 + ways( idx, val1 + coins[idx].fi, val2 + coins[idx].se ) );
...
int main(void){
  ...
  while( tc -- ){
    ...
    ll ans = ways( m - 1, 0, 0 );
    if( ans == LLINF ) cout << "not possible\n";
    else cout << ans << '\n';
  }
  return 0;
}
