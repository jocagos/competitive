...
int main(void){
  fastio;
  int heads, knights;
  vi dragon(20000, 0), kingdom(20000, 0);
  while( cin >> heads >> knights, heads | knights ){
    int cost = 0, d = 0, k = 0;
    REP( i, heads ) cin >> dragon[i];
    REP( i, knights ) cin >> kingdom[i];
    sort( justN( dragon, heads ) );
    sort( justN( kingdom, knights ) );
    while( d < heads and k < knights ){
      while( dragon[d] > kingdom[k] and k < knights ) ++ k;
      if( k == knights ) break;
      cost += kingdom[k];
      ++ d, ++ k;
    }
    if( d == heads ) cout << cost << '\n';
