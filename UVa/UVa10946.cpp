...
int bfs( int x, int y, char c ){
  ...
  while( not q.empty() ){
    ...
    if( g[u.fi][u.se] == c ) s ++;
    for( auto p : n4 ){
      if( in( 0, n - 1, u.fi + p.fi ) and in( 0, ::m - 1, u.se + p.se ) ){
	if( not v[u.fi + p.fi][u.se + p.se] and g[u.fi + p.fi][u.se + p.se] == c ) q.emplace( u.fi + p.fi, u.se + p.se );
      }
    }
  }
  return s;
...
int main(void){
  ...
  while( cin >> n >> m, n | m ){
	...
	if( not v[i][j] and g[i][j] != '.' ){
	  int tmp = bfs( i, j, g[i][j] );
	  cc.emplace( g[i][j], tmp );
	}
      }
    }
    for( auto p : cc ) val.emplace_back( p.fi, p.se );
    sort( all( val ), []( pair<char, int> left, pair<char, int> right ){
			if( left.se != right.se ) return left.se > right.se;
			else // if( left.fi != right.fi )
			  return left.fi < right.fi;
		      } );
    cout << "Problem " << problem ++ << ":\n";
    for( auto p : val ) cout << p.fi << " " << p.se << "\n";
    memset( g, 0, sizeof g );
    memset( v, 0, sizeof v );
    cc.clear();
    val.clear();
