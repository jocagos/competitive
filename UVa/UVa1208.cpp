...
int tc, n, m, k, a, b, c;
vtriad<int> edges( MAXN * MAXN ), mst;
int g[MAXN][MAXN];
...
int main(void){
  ...
  FOR( _, 1, tc + 1 ){
	...
	g[i][j] = c;
	if( c ) edges[edx++] = MT( c, i, j );
      }
    }
    sort( begin( edges ), begin( edges ) + edx );
    UFDS ufds( edx );
    REP( i, edx ){
    ...
    for( auto t : mst ){
      cout << char(min(get<1>(t), get<2>(t)) + 'A') << "-" << char(max(get<1>(t), get<2>(t)) + 'A') << " " << get<0>(t) << '\n';
    }
  }
  return 0;
}
