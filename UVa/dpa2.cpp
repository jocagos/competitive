...
string compute( long long n ){
  ...
  string ans;
  if( sum == cache ) return ans = to_string(cache) + " " + to_string(sum) + " perfect";
  else if( sum > cache ) return ans = to_string(cache) + " " + to_string(sum) + " abundant";
  else return ans = to_string(cache) + " " + to_string(sum) + " deficient";
}
