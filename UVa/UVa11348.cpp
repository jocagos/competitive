...
int main(void){
  ...
  FOR( _, 1, tc + 1 ){
    ...
    REP( i, n ){
      ...
      REP( j, x ){
	cin >> input[i][j];
	if( already.find( input[i][j] ) == already.end() ) keys[input[i][j]] ++;
	already.emplace( input[i][j] );
	if( keys[input[i][j]] > 1 ) repeated.emplace( input[i][j] );
      }
    }
    cout << "Case " << _ << ":";
    // cerr << "Case " << _ << ":";
    REP( i, n ){
      ...
      int a = (int)keys.size();
      int k = 0;
      REP( j, (int)input[i].size() ){
	if( repeated.find( input[i][j] ) == repeated.end() and already.find( input[i][j] ) == already.end() ){
	  k ++;
	  already.emplace( input[i][j] );
	}
      }
