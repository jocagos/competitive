...
bitset<100> reachable(0), iteration(0);
vector<vector<int>> adjList( 100, vector<int>() );

// Marcamos los que podemos llegar desde el 0
void dfsReached( int u ){
  reachable[u] = true;
  REP( _i, (int)adjList[u].size() ){
    int v = adjList[u][_i];
   if( !reachable[v] ) dfsReached( v );
  }
}

// Marcamos los que podemos llegar desde el 0 SIN nodo `off`
void dfs( int u, int off ){
  if( u == off ) return;
  iteration[u] = true;
  REP( _i, (int)adjList[u].size() ){
    int v = adjList[u][_i];
    if( !iteration[v] ) dfs( v, off );
  }
}
...
int main(void){
  ...
  fastio;
  cin >> t;
  // bool dominators[100][100];
  FOR( i, 1, t + 1 ){
    // memset( dominators, 0, sizeof dominators );
    cin >> n;
    short node = 0;
    REP( j, n ){
      // dominators[j][j] = true; // every node dominates itself
      REP( k, n ){
	cin >> node;
	if( node ) adjList[j].EB( k );
      }
    }
    // Mark where we can get from 0
    dfsReached( 0 );
    string s(2 * n - 1, '-');
    s = "+" + s + "+\n";
    cout << "Case " << i << ":\n";
    for( int off = 0; off < n; ++ off ){
      // then iteratively get the right answer one by one
      dfs( 0, off );
      cout << s;
      REP( j, n ) cout << "|" << ( reachable[j] ^ iteration[j] ? "Y" : "N" );
      if( n ) cout << "|\n";
      iteration.reset();
    }
    if( n ) cout << s;
    reachable.reset();
    REP( j, (int)adjList.size() ) adjList[j].clear();
  }
  return 0;
}
