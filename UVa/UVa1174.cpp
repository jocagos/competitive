...
constexpr int MAXN = 50050, MAXM = 2020;
int m, n, a, b, c, tc, idx = 0;
string A, B;
...
int main(void){
  ...
  while( tc -- ){
    ...
    REP( i, n ){
      cin >> A >> B >> c;
      if( cities.find( A ) == cities.end() ) cities[A] = idx ++;
      if( cities.find( B ) == cities.end() ) cities[B] = idx ++;
      a = cities[A], b = cities[B];
      edges[i] = MT( c, a, b );
    }
    sort( begin(edges), begin(edges) + n );
    UFDS ufds( n );
    int cost = 0;
    REP( i, n ){
