...
void backtrack( int bitmask = 0, int sum = 0 ){
  // int s = 0;
  // for( int i = 0; i < n; ++ i ) if( bitmask & (1 << i) ) s +=
  if( sum == t ){
    total++;
    ans.emplace( bitmask );
    return;
  ...
  REP( i, n ){
    if( !( bitmask & (1 << i) ) ) backtrack( bitmask | (1 << i), sum + a[i] );
  }
}
...
int main(void){
  ...
  while( cin >> t >> n, t | n ){
    ...
    backtrack( 0, 0 );
    cout << "Sums of " << t << ":\n";
    // There's a bug! You get the same value more than once!
    if( ans.size() ){
      for( auto bitmask : ans ){
	int bits = 0;
	for( int i = 0; i <= n; ++ i ){
	  if( bitmask & (1 << i) ){
	    cout << a[i];
	    if( bits < cntSetBits( bitmask ) - 1 )cout << "+";
	    bits ++;
	  }
	}
	cout << '\t';
