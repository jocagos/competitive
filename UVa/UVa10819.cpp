...
constexpr int MAXN = 110, MAXM = 10210;
vii items( MAXN );
ll dp[MAXN][MAXM];
int n, m;

ll ans( int idx, int cost ){
  if( idx == n ){
    if( cost <= m or ( cost > 2000 and cost <= m + 200 ) ) return 0;
    else return -20000;
  }
  if( dp[idx][cost] == -1 ){
    ll sec = -1;
    if( items[idx].fi + cost <= m + 200 ) sec = ans( idx + 1, items[idx].fi + cost ) + items[idx].se;
    dp[idx][cost] = ans( idx + 1, cost );
    if( sec >= 0 or dp[idx][cost] >= 0 ) dp[idx][cost] = max( sec, dp[idx][cost] );
  }
  return dp[idx][cost];
}
...
int main(void){
  fastio;
  while( cin >> m >> n ){
    REP( i, n ) cin >> items[i].fi >> items[i].se;
