...
constexpr triad<int> n3d[6] = { { 0, -1, 0 }, // up
		     { 0, 1, 0 }, // down
		     { -1, 0, 0 }, // left
		     { 1, 0, 0 }, // right
		     { 0, 0, -1 }, // backward
...
// 1st version: hard code all states because it's doable
// complexity (space-wise): MAXN * MAXN * MAXT ~= 6M => O(t * n^2)
// key idea: block spaces when they are going to be monitored
// movements: forward, left, right, wait (no back, it's never useful)
constexpr int MAXN = 110, MAXT = 550;
constexpr int NORTH = 1, WEST = 2, SOUTH = 4, EAST = 8;
// use a bitmask in every point in this way:
// 1 = North
// 2 = West
// 4 = South
// 8 = East
// this way we can know where to go just by looking at the bitmask
// iterating through it with the following pseudo-enum:
constexpr int dir[] = { NORTH, WEST, SOUTH, EAST };
constexpr int m2n4[] = { -1, 3, 1, -1, 0, -1, -1, -1, 2 };
// and checking like: if( dir[i] & grid[x][y][t] ) q.emplace( x + g0(n4[m2n4[dir[i]]]), y + g1(n4[m2n4[dir[i]]]), t + 1 );
int grid[MAXN][MAXN][MAXT];
bool can[MAXN][MAXN]

int main(void){
  int col, row, r, x1, x2, y1, y2, m, t, x, y;
  fastio;
  while( cin >> col >> row ){
    cin >> r;
    // build graph here so we disconnect in the next step
    // turn on all bits
    memset( grid, 0x0F, sizeof grid ); // 0x0F = 15
    REP( i, r ){
      cin >> x1 >> y1 >> x2 >> y2;
      int dir_bitmask = 0;
      if( x1 < x2 ) dir_bitmask = EAST;
      else if( x1 > x2 ) dir_bitmask = WEST;
      else if( y1 < y2 ) dir_bitmask = NORTH;
      else if( y1 > y2 ) dir_bitmask = SOUTH;
      REP( _, MAXT )
	grid[x1][y1][_] ^= dir_bitmask; // turn it off
    }
    cin >> m;
    REP( i, m ){
      cin >> t >> x >> y;
      grid[x][y][t] |= 0xF0; // add 0xF0 to make it known as a stop
    }
    queue<triad<int>> q;
    int layer = 0;
    q.emplace( 0, 0, layer );
	...
	break;
      ...
      for( auto d : dir ){
	int dx = get<0>(n4[m2n4[d]]), dy = get<1>( n4[m2n4[d]] );
	if( in( 0, col - 1, x + dx ) and in( 0, row - 1, y + dy ) and !(grid[x + dx][y + dy][t + 1] & 0xF0) ) q.emplace( x + dx, y + dy, t + 1 );
      }
      if( !(grid[x][y][t + 1] & 0xF0 ) ) q.emplace( x, y, t + 1 );
    }
    cout << layer << '\n';
  }
  return 0;
