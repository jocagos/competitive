...
vtriad<int, string, string> edges( MAXN * MAXN );// , mst;
vs nodes( MAXN );
unordered_map<string, int> idxs;
int n, m, idx = 0;

inline int d( string left, string right ){
  int a = 0;
  REP( i, min(left.length(), right.length()) ) a += min( abs(left[i] - right[i]), 10 - abs(left[i] - right[i]));
  return a;
}
...
int main(void){
  ...
  while( tc -- ){
    ...
    idxs[nodes[n]] = n;
    idx = 0;
    tuple<int,string,string> t { 50, "0000", "AAAA" };
    REP( i, n ){
      int dist = d( nodes[n], nodes[i] );
      if( dist < get<0>( t ) ) t = MT( dist, "0000", nodes[i] );
    }
    edges[idx++] = t;
    REP( i, n ){
      FOR( j, i + 1, n ){
	edges[idx++] = { d( nodes[i], nodes[j] ), nodes[i], nodes[j] };
      }
    }
    sort( begin( edges ), begin( edges ) + idx );
    // for( int i = 0; i < idx; ++ i ){
    //   auto t = edges[i];
    ...
    REP( i, idx ){
      ...
      if( not ufds.isSameSet( idxs[get<1>( edges[i] )], idxs[get<2>( edges[i] )] ) ){
	cost += get<0>( edges[i] );
	ufds.unionSet( idxs[get<1>( edges[i] )], idxs[get<2>( edges[i] )] );
	// mst.PB( edges[i] );
      }
    }
    cout << cost << '\n';
    // for( auto t : mst ) cout << "(" << get<0>( t ) << ", " << get<1>( t ) << ", " << get<2>( t ) << ")\n";
  }
  return 0;
}
