...
inline ld p( int n, int k, int v ){
  if( 1.0L * n / k - v > 0 ) return 0.3L * k * sqrt( 1.0L * n / k - v );
  else return 0.0L;
}
...
int main(void){
  ...
  while( cin >> v >> v0, v | v0 ){
    ...
    int p = 0;
    FOR( k, 1, v + 1 ){
    // while( not stackoverflow and 1.0L * v / k > v0 and k <= (v + 1) / 2 ){
      double disc = 1.0L * v / k, key;
      if( disc - v0 > 0 ) key = 0.3 * sqrt( disc - v0 ) * k;
      else break;
      string s = to_string( key ), t = to_string( l );
      if( s == t ) p = 0;
      else if( key > l ) p = k, l = key;
      else if( key < l ) break;
    }
    cout << p << endl;
  }
