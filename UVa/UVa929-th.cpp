...
// Using threads!
const unsigned int MAXTH = thread::hardware_concurrency();
...
int main(void){
  ...
  while( tc -- ){
    ...
    while( not pq.empty() ){
      ...
      if( MAXTH >= 3 ){
	auto checker = [&x, &y, &pq, &dist, &n4]( int idx ){
			 int diff_x, diff_y;
			 tie( diff_x, diff_y ) = n4[idx];
			 if( in( 0, n - 1, x + diff_x ) and in( 0, m - 1, y + diff_y ) and
			     dist[x][y] + grid[x + diff_x][y + diff_y] < dist[x + diff_x][y + diff_y] ){
			   dist[x + diff_x][y + diff_y] = dist[x][y] + grid[x + diff_x][y + diff_y];
			   triad<int> tmp = { dist[x + diff_x][y + diff_y], x + diff_x, y + diff_y };
			   pq.push( tmp );
			 }
		       };
	for( int i = 0; i < 2; ++ i ){
	  thread a( checker, i );
	  thread b( checker, i + 2 );
	  a.join();
	  b.join();
	}
      }
