...
template<typename T>
bool in( T const& left, T const& right, T const& val ){
...
constexpr int MAXN = 1010, BOMB = -1;

int r, c, n, k, l, tmp, sx, sy, dx, dy;
int g[MAXN][MAXN];
...
int main(void){
  ...
  while( cin >> r >> c, r | c ){
    ...
    g[dx][dy] = '*';
#ifdef DBG
    cerr << "r: " << setw( 4 ) << r << " c: " << setw( 4 ) << c << endl;
    cerr << "n: " << setw( 4 ) << n << endl;
    cerr << "sour(" << setw( 4 ) << sx << ", " << setw( 4 ) << sy << ")" << endl;
    cerr << "dest(" << setw( 4 ) << dx << ", " << setw( 4 ) << dy << ")" << endl;
    for( int _ = 0; _ < r; ++ _ ){
      for( int __ = 0; __ < c; ++ __ ){
	if( (_ == sx and __ == sy) or (_ == dx and __ == dy) )
	  cerr << setw( 2 ) << (char)g[_][__];
	else
	  cerr << setw( 2 ) << g[_][__];
      }
      cerr << endl;
...
#ifdef DBG
      cerr << "Checking position (" << setw( 4 ) << ux << ", " << setw( 4 ) << uy << ") = " << setw( 4 ) << g[ux][uy] << endl;
#endif
      if( g[ux][uy] == BOMB ) continue;
      // c++17 only
      // for( const auto [x, y] : n4 ){
      // 	if( in( 0, c, ux + x ) and in( 0, r, uy + y ) and g[ux + x][uy + y] == 0 and dist[ux + x][uy + y] == INF and g[ux + x][uy + y] != BOMB ) q.emplace( ux + x, uy + y ), dist[ux + x][uy + y] = dist[ux][uy] + 1;
      // }
      // c++11 :( - c'mon uva support more recent compilers
      for( int i = 0; i < 4; ++ i ){
	int x = get<0>( n4[i] ), y = get<1>( n4[i] );
	// cerr << "x: " << x << "\ty: " << y << endl;
	if( in( 0, r - 1, ux + x ) and
	    in( 0, c - 1, uy + y ) and
	    dist[ux + x][uy + y] == INF and
	    g[ux + x][uy + y] != BOMB ){
#ifdef DBG
	  cerr << "g[" << ux + x << "][" << uy + y << "] = " << g[ux + x][uy + y] << " was emplaced with dist = " << dist[ux][uy] + 1 << endl;
#endif
	  q.emplace( ux + x, uy + y );
	  dist[ux + x][uy + y] = dist[ux][uy] + 1;
	}
	// if( ux + x == dx and uy + y == dy ) found = true;
      }
      g[ux][uy] = dist[ux][uy];
      // if( ux == dx and uy == dy ) break;
    }
