...
constexpr int MAXN = 250; // modify
int n, tc, xs, ys, xt, yt, xsub, ysub;
string line;
vtwin<int> points( MAXN );
vi subway_line( MAXN, 0 );
map<twin<int>, int> ipoints;
int idx = 0;
...
int main(void){
  ...
  while( tc -- ){
    ...
    while( getline( cin, line ) ){
      ...
      xsub = ysub = 0;
      while( iss >> xsub >> ysub ){
	if( xsub == -1 and ysub == -1 ) break;
	points[idx] = { xsub, ysub };
	ipoints[points[idx]] = idx;
	subway_line[idx] = subline;
	idx ++;
      }
    }
    points[idx] = { xt,  yt };
    ...
    map<int, double> dist;
    dist[0] = 0;
    FOR( i, 1, idx ) dist[i] = INF;
    tuple<double, int> tmp{ dist[0], 0 };
    std::priority_queue< tuple<double,int>, vector<tuple<double, int>>, greater<tuple<double, int>>> pq;
    pq.push( tmp );
    while( not pq.empty() ){
      tmp = pq.top();
      pq.pop();
      ...
      FOR( i, 0, idx ){
	// if( get<0>(points[i]) == x and get<1>(points[i]) == y ) continue;
	double new_d = hypot( get<0>(points[i]) - x, get<1>(points[i]) - y );
	if( subway_line[a] != subway_line[i] ) new_d *= 4;
	if( dist[i] > new_d + d ){
	  dist[i] = new_d + d;
	  pq.emplace( new_d + d, i );
	}
      }
    } // end while
    cout << round(dist[idx-1] * 1.5 / 1000) << (tc ? "\n\n" : "\n");
  }
  return 0;
}
