...
constexpr char EMPTY = '.', ROCK = '#', START = 'S', EXIT = 'E', EXPLORING = '*';

char dung[MAXN][MAXN][MAXN];
int l, r, c;
int sx, sy, sz, dx, dy, dz;
string line;
string success1("Escaped in "), success2(" minute(s)."), failure("Trapped!");
triad<int> n3d[] = { { 0, -1, 0 }, // up
		     { 0, 1, 0 }, // down
		     { -1, 0, 0 }, // left
		     { 1, 0, 0 }, // right
		     { 0, 0, -1 }, // backward
...
int main(void){
  ...
  while( getline( cin, line ) ){
	...
	REP( k, c ){
	  dung[i][j][k] = line[k];
	  if( dung[i][j][k] == START ) sx = i, sy = j, sz = k;
	  if( dung[i][j][k] == EXIT ) dx = i, dy = j, dz = k;
	}
      }
      getline( cin, line ); // read blank space between levels
    } // end levels
#ifdef DBG
    cerr << "l = " << l << "\tc = " << c << "\tr = " << r << endl;
    cerr << "Source is ( " << sx << ", " << sy << ", " << sz << " )" << endl;
    cerr << "Destination is ( " << dx << ", " << dy << ", " << dz << " )" << endl;
#endif
    int d = INF;
    queue<quad<int>> q;
    // map<int, int> by_layer;
    // int emp_per_layer = 0;
    q.emplace( sx, sy, sz, 0 );
    while( not q.empty() ){
      // emp_per_layer = 0;
      int cx, cy, cz, layer;
      tie( cx, cy, cz, layer ) = q.front();
#ifdef DBG
      cerr << "( " << cx << ", " << cy << ", " << cz << " ) emplaced { ";
#endif
      q.pop();
      if( cx == dx and cy == dy and cz == dz ){
	d = layer;
	break;
      }
      dung[cx][cy][cz] = ROCK; // Explored
#ifdef DBG
      vector<quad<int>> emplaced;
#endif
      for( auto tern : n3d ){
	int x, y, z;
	tie( x, y, z ) = tern;
	// if( cx + x == 29 and cy + y == 29 and cz + z == 29 ){
	//   cerr << "WE GOT HERE BOIS" << endl;
	//   cerr << "FROM: (" << cx << ", " << cy << ", " << cz << ")" << endl;
	// }
	if( in( 0, l - 1, cx + x ) and in( 0, r - 1, cy + y ) and in( 0, c - 1, cz + z ) and isIn( dung[cx + x][cy + y][cz + z], { EMPTY, EXIT }) ){
	  q.emplace( cx + x, cy + y, cz + z, layer + 1 );
	  dung[cx + x][cy + y][cz + z] = EXPLORING; // mark to avoid re-exploring
#ifdef DBG
	  emplaced.EB( cx + x, cy + y, cz + z, layer + 1 );
#endif
	}
      }
#ifdef DBG
      for( int i = 0; i < (int)emplaced.size(); ++ i ){
	int _x, _y, _z, _l;
	tie( _x, _y, _z, _l ) = emplaced[i];
	cerr << "(" << _x << ", " << _y << ", " << _z << ", " << _l << ")" << (i + 1 == (int)emplaced.size() ? "" : ", ");
      }
      cerr << " }" << endl;
#endif
      // by_layer[layer] += emp_per_layer;
    } // end while
    // for( auto p : by_layer ) cerr << "Layer " << p.first << " emplaced " << p.second << endl;
    cout << (d == INF ? failure : success1 + to_string( d ) + success2) << '\n';
  }
  return 0;
}
