...
constexpr int MAXN = 30010, MAXC = 5;
int coins[] = { 50, 25, 10, 5, 1 };
int n;
ll dp[MAXC][MAXN];

ll ways( ll idx, ll val ){
  if( val == 0 ) return 1;
  if( val < 0 or idx == MAXC ) return 0;
  if( dp[idx][val] != -1 ) return dp[idx][val];
  return dp[idx][val] = ways( idx + 1, val ) + ways( idx, val - coins[idx] );
}
...
int main(void){
  ...
  while( cin >> n ){
    ll ans = ways( 0, n );
    cout << "There " << ( ans == 1 ? "is only " : "are " ) << ans << ( ans == 1 ? " way" : " ways" ) << " to produce " << n << " cents change.\n";
  }
  return 0;
}
