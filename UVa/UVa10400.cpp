...
constexpr int MAXN = 110, MAXV = 32010, OFF = 32001;
int n, target;
int a[MAXN];
string r;
bool reached[MAXN][MAXV*2];

bool backtrack( int next, int current, string s ){
  if( next == n ){
    if( current == target ){
      r = s;
      return true;
  ...
  if( not in( -32000, 32000, current ) ) return false;
  if( reached[next][current + OFF] ) return false;
  reached[next][current + OFF] = true;
  if( backtrack( next + 1, current + a[next], s + "+" ) ) return true;
  if( backtrack( next + 1, current - a[next], s + "-" ) ) return true;
  if( backtrack( next + 1, current * a[next], s + "*" ) ) return true;
  if( current % a[next] == 0 ) if( backtrack( next + 1, current / a[next], s + "/" ) ) return true;
  return false;
}
...
int main(void){
  ...
  while( tc -- ){
    ...
    if( r == "NO EXPRESSION\n" ) cout << r;
      ...
      cout << a[0];
      for( int i = 0; i < r.length(); ++ i ) cout << r[i] << a[i+1];
      cout << "=" << target << '\n';
    }
  }
  return 0;
